...
(function(){
  ...
  function relabelOptions(root=document){for(const opt of root.querySelectorAll?.("select option")||[]){const meta=metaFor(opt.value);if(meta){const label=full(meta);if(opt.textContent!==label)opt.textContent=label}}}
  function relabelHeadings(root=document){const replacements=new Map([["Class & Section","Grade / Section / Orientation"],["Class/Section","Grade / Section"],["Class/Sec","Grade / Section"],["Class","Grade / Section"]]);for(const el of root.querySelectorAll?.("label,th,.eyebrow,.panel-head h3,.metric small")||[]){const t=txt(el.textContent);if(replacements.has(t)){const label=replacements.get(t);if(el.textContent!==label)el.textContent=label}}}
  function relabelLegacyText(root=document){const map=new Map(allSections().map(s=>[txt(s.section),compact(s)]).filter(([k])=>k));if(!map.size)return;const walker=document.createTreeWalker(root.body||root,NodeFilter.SHOW_TEXT,{acceptNode(node){const p=node.parentElement;if(!p||["SCRIPT","STYLE","OPTION","TEXTAREA","INPUT"].includes(p.tagName))return NodeFilter.FILTER_REJECT;return map.has(txt(node.nodeValue))?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT}});const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);for(const n of nodes)n.nodeValue=map.get(txt(n.nodeValue))}
