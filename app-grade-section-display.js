...
(function(){
  ...
  function sectionCode(meta){
    if(!meta)return"";
    const raw=txt(meta.batch)||txt(meta.section);
    const techno=raw.match(/^\d+(?:st|nd|rd|th)\s+techno$/i);
    return techno?"Techno":raw
  }
  function compact(meta){return meta?`${ordinal(meta.grade)} · ${sectionCode(meta)}`:""}
  function full(meta){
    if(!meta)return"";
    return [ordinal(meta.grade),sectionCode(meta),txt(meta.program)].filter(Boolean).join(" · ")
  }
  ...
  function relabelOptions(root=document){
    for(const opt of root.querySelectorAll?.("select option")||[]){
      const meta=metaFor(opt.value);if(meta)opt.textContent=full(meta)
    }
  }
  function relabelHeadings(root=document){
    const replacements=new Map([
      ["Class & Section","Grade / Section / Orientation"],
      ["Class/Section","Grade / Section"],
      ["Class/Sec","Grade / Section"],
      ["Class","Grade / Section"]
    ]);
    const els=root.querySelectorAll?.("label,th,.eyebrow,.panel-head h3,.metric small")||[];
    for(const el of els){const t=txt(el.textContent);if(replacements.has(t))el.textContent=replacements.get(t)}
  }
  function relabelLegacyText(root=document){
    const map=new Map(allSections().map(s=>[txt(s.section),compact(s)]).filter(([k])=>k));
    if(!map.size||!root.body&&!root.nodeType)return;
    const walker=document.createTreeWalker(root.body||root,NodeFilter.SHOW_TEXT,{acceptNode(node){
      const p=node.parentElement;if(!p||["SCRIPT","STYLE","OPTION","TEXTAREA","INPUT"].includes(p.tagName))return NodeFilter.FILTER_REJECT;
      const t=txt(node.nodeValue);return map.has(t)?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT
    }});
    const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);
    for(const n of nodes)n.nodeValue=map.get(txt(n.nodeValue))
  }
  function apply(){
    relabelOptions(document);relabelHeadings(document);relabelLegacyText(document)
  }
  ...
  if(typeof applyRemoteData==="function"){
    const previous=applyRemoteData;applyRemoteData=function(r){const out=previous(r);queue();return out}
  }
  if(typeof renderAll==="function"){
    const previous=renderAll;renderAll=function(){const out=previous();queue();return out}
  }
