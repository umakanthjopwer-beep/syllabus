...
(function(){
  ...
  function subjectLine(r){
    const all=(REPORT_STATE.subject||REPORT_ALL_SUBJECTS)===REPORT_ALL_SUBJECTS;
    const second=all?`<small class="report-row-subject">${reportEsc(r.subject||"")}</small>`:`<small>${reportEsc(r.batch||r.program||"")}</small>`;
    return `<b>${reportEsc(r.section)}</b>${second}`
  }
  function onePage(rows,page,totalPages,offset){
    ...
    return `<div class="weekly-print-sheet readable-report-page">
      ...
      <table class="weekly-paper-table readable-report-table"><thead><tr>
        ...
        <td>${offset+i+1}</td><td>${subjectLine(r)}</td>
  ...
  function styles(){
    ...
    const s=document.createElement("style");s.id="readableReportPrintStyles";s.textContent=`
      .readable-report-pages{display:grid;gap:18px}.readable-report-page{margin-bottom:0!important}.report-row-subject{font-weight:700;color:#4b5f79!important}
      ...
      @media screen{.readable-report-page{width:1120px;max-width:100%;min-width:900px}}
      @media print{
