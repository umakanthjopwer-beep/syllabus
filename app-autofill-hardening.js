...
(function(){
  ...
  function textDate(d,m,y){let yy=Number(y);if(yy<100)yy+=2000;const mm=Number(m),dd=Number(d);if(!yy||mm<1||mm>12||dd<1||dd>31)return"";const x=new Date(Date.UTC(yy,mm-1,dd));return x.getUTCFullYear()===yy&&x.getUTCMonth()===mm-1&&x.getUTCDate()===dd?ISO(x):""}
  function embeddedDateSpan(text){const hits=[...clean(text).matchAll(/\b(\d{1,2})[.\-/](\d{1,2})[.\-/](\d{2,4})\b/g)].map(m=>textDate(m[1],m[2],m[3])).filter(Boolean);if(hits.length<2)return null;let a=hits[0],b=hits[1];if(b<a){const t=a;a=b;b=t}return{start:a,end:b}}
  ...
  function recoveredAggregate(rows,w){
    const direct=(rows||[]).filter(r=>r.startDate&&overlaps(r.startDate,r.endDate||r.startDate,w.start,w.end)),carried=[];
    for(const r of rows||[]){const span=embeddedDateSpan(r.topic);if(!span||!overlaps(span.start,span.end,w.start,w.end)||!clean(r.topic))continue;carried.push({...r,startDate:w.start,endDate:w.end,workingDays:null,plannedPeriods:null,_sourceKey:`dated-event|${span.start}|${span.end}|${clean(r.topic)}`})}
    const row=bestRows([...direct,...carried]).find(r=>r.startDate===w.start);return{topic:clean(row?.topic),workingDays:row?.workingDays??null,plannedPeriods:row?.plannedPeriods??null}
  }
  ...
  async function rescueSelectedWeek(section,subject,w){
    ...
    const task=(async()=>{
      const plans=matchingPlans(section,subject);if(!plans.length)return false;setSourceNote("Stored weekly row is missing or incomplete. Reading the original Year Plan automatically…");
  ...
  fillWeeklyCalendarFromPlan=function(preserveSelection=true){
    const result=baseFill(preserveSelection);setTimeout(()=>{try{const section=document.getElementById("wkSection")?.value,subject=canonicalSubject(document.getElementById("wkSubject")?.value||""),w=selectedCalendarWeek?.(),box=document.getElementById("wkPlanned"),days=document.getElementById("wkDays");if(!section||!subject||!w)return;const x=planRowsFor(section,subject);if(!x.plan)return;if(clean(box?.value)&&clean(days?.value))return;rescueSelectedWeek(section,subject,w)}catch(e){console.warn("Weekly syllabus autofill rescue",e)}},0);return result
  };
