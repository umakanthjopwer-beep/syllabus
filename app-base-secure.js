...
const LEGACY_MAPPINGS=LEGACY_SEED.mappings.map((r,i)=>({
  id:"legacy-"+(i+1),section:r[0],subject:r[1],teacher:r[2],periodsPerWeek:Number(r[3]||0),week:r[4]||"Every Week",
  department:r[5]||"",classTeacher:r[6]||"",coClassTeacher:r[7]||"",originalSubject:r[8]||r[1],
  activeForSyllabus:!!r[5]
}));
...
function canonicalSubject(s){
  const x=norm(s);
  if(["Maths Track A","Math A","Maths A"].includes(x)) return "Track A";
  if(["Maths Track B","Math B","Maths B"].includes(x)) return "Track B";
  if(x==="A&R"||x==="Arithmetic & Reasoning"||x==="Reasoning & Arithmetic") return "Reasoning";
  return x;
}
function departmentForSubject(s){
  const c=canonicalSubject(s);
  return DEPARTMENT_ORDER.find(d=>DEPARTMENTS[d].includes(c))||(["Lead Activity"].includes(c)?"Social":"");
}
function statusOf(row){
  const p=Number(row.lagPeriods||0);
  if(row.notApplicable) return "Not Applicable";
  if(p===0) return "On Track";
  if(p<=2) return "Minor Lag";
  if(p<=5) return "Lagging";
  return "Critical Lag";
}
...
function defaultData(){
  return {
    ...
    plans:[],weekly:[],
    users:[
      {id:"u-super",name:"J Umakanth",username:"umakanth",password:"",passwordHash:"44f65add6da43c99b8f0ca91d7c452fd783fcdd732146ac99b6edeb11e44d74c",role:"Super Admin",department:"Mathematics",departments:["Mathematics"],sections:SECTIONS.map(x=>x.section),subjects:ALL_SUBJECTS,accessEnabled:true,mustChangePassword:false},
      {id:"u-hod-social",name:"B. Aruna",username:"aruna",password:"",role:"HOD",department:"Social",departments:["Social"],sections:[],subjects:DEPARTMENTS.Social,accessEnabled:false,mustChangePassword:true}
    ],
    setup:{orientations:[{name:"C Batch",active:true},{name:"Lead",active:true},{name:"Techno",active:true}],teachers:clone(LEGACY_TEACHERS),handlingMappings:clone(LEGACY_MAPPINGS),sourceCounts:clone(LEGACY_SEED.sourceCounts),migratedFrom:"Khalsa_Timetable_Full_Bulk_Import_FILLED.xlsx",bulkImports:[]},
    uiScope:{departments:[],sections:[],subjects:[]}
  };
}
function migrateData(d){
  const base=defaultData();const out={...base,...(d||{})};out.setup={...base.setup,...(d?.setup||{})};out.uiScope={...base.uiScope,...(d?.uiScope||{})};
  out.plans=(d?.plans||[]).map(p=>({...p,subject:canonicalSubject(p.subject),department:p.department||departmentForSubject(p.subject),assignedSections:Array.isArray(p.assignedSections)&&p.assignedSections.length?p.assignedSections:[p.section].filter(Boolean),enabled:p.enabled!==false}));
  out.weekly=(d?.weekly||[]).map(w=>({...w,subject:canonicalSubject(w.subject),department:w.department||departmentForSubject(w.subject)}));
  const userMap=new Map((d?.users||base.users).map(u=>[String(u.username||u.id).toLowerCase(),{...u}]));if(!userMap.has("umakanth"))userMap.set("umakanth",base.users[0]);
  const uma=userMap.get("umakanth");if(!uma.password&&!uma.passwordHash)uma.passwordHash="44f65add6da43c99b8f0ca91d7c452fd783fcdd732146ac99b6edeb11e44d74c";uma.accessEnabled=uma.accessEnabled!==false;uma.role="Super Admin";
  out.users=[...userMap.values()];
  if(!Array.isArray(out.setup.teachers)||out.setup.teachers.length<30)out.setup.teachers=clone(LEGACY_TEACHERS);
  if(!Array.isArray(out.setup.handlingMappings)||out.setup.handlingMappings.length<200)out.setup.handlingMappings=clone(LEGACY_MAPPINGS);
  out.setup.sourceCounts=out.setup.sourceCounts||clone(LEGACY_SEED.sourceCounts);out.schemaVersion=APP_SCHEMA;return out;
}
...
let data=loadData();
function persist(){data.schemaVersion=APP_SCHEMA;localStorage.setItem(STORE_KEY,JSON.stringify(data))}
...
function injectEnhancements(){
  ...
  if(!$("#departmentNav")){
    const sideBtn=document.createElement("button");sideBtn.id="departmentNav";sideBtn.className="nav-btn hidden";sideBtn.dataset.view="department";sideBtn.innerHTML="<span>M</span>My Department";$("#sideNav").insertBefore(sideBtn,$('#sideNav [data-view="weekly"]'));
    const mob=document.createElement("button");mob.id="departmentMobileNav";mob.className="hidden";mob.dataset.view="department";mob.innerHTML="<span>M</span><small>My Dept</small>";$("#mobileNav").insertBefore(mob,$('#mobileNav [data-view="weekly"]'));
    [sideBtn,mob].forEach(b=>b.onclick=()=>showView("department"));
  }
  if(!$("#planEditDialog")){
    document.body.insertAdjacentHTML("beforeend",`<dialog id="planEditDialog" class="app-dialog"><form id="planEditForm" class="dialog-card"><div class="dialog-head"><div><div class="eyebrow">EDIT YEAR PLAN</div><h3>Assignments & mapping</h3></div><button type="button" id="closePlanEditBtn" class="icon-close">×</button></div><div class="form-grid"><label>Department<select id="editPlanDepartment"></select></label><label>Subject<select id="editPlanSubject"></select></label><label class="full">Handling Teacher <small class="small-muted">Teacher assignment normally comes from Handling Classes. This field is an optional plan-level reference.</small><input id="editPlanTeacher"></label></div><div class="scope-block"><div class="scope-title"><strong>Assigned Classes / Sections</strong><button type="button" id="editPlanSelectAll" class="mini-btn">Select all applicable</button></div><div id="editPlanSections" class="choice-grid four"></div></div><p class="small-muted">Adding or removing a section changes future Year Plan assignment only. Existing weekly submissions and historical reports are not deleted.</p><div class="row-actions"><span id="editPlanStatus" class="status-text"></span><button type="submit" class="primary">Save changes</button></div></form></dialog>`);
    $("#closePlanEditBtn").onclick=()=>$("#planEditDialog").close();
    $("#editPlanDepartment").onchange=()=>fillSelect($("#editPlanSubject"),DEPARTMENTS[$("#editPlanDepartment").value]||[],$("#editPlanSubject").value);
    $("#editPlanSelectAll").onclick=()=>$$('#editPlanSections input[type="checkbox"]').forEach(i=>{i.checked=true;i.closest(".choice-card")?.classList.add("selected")});
