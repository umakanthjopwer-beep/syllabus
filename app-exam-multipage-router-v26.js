// Generic multi-page exam PDF router v26.
...
(function(){
  ...
  function pageMeta(lines){
    const joined=txt(lines.join(" "));
  ...
  async function readPages(file){const lib=await pdfLib(),pdf=await lib.getDocument({data:await file.arrayBuffer()}).promise,pages=[];for(let p=1;p<=pdf.numPages;p++){const pg=await pdf.getPage(p),tc=await pg.getTextContent(),items=[];for(const it of tc.items||[]){const v=txt(it.str);if(!v)continue;items.push({v,x:Number(it.transform?.[4]||0),y:Number(it.transform?.[5]||0),w:Number(it.width||0),h:Number(it.height||0)})}const rows=groupRows(items),lines=rows.map(r=>rowText(r.items)).filter(Boolean);pages.push({page:p,rows,lines})}return pages}
  ...
  function makeGroups(pages){
    ...
    for(const page of pages){const meta=pageMeta(page.lines);if(!meta)continue;const parsed=parsePageTopics(page),mappings=mappingsFor(meta,parsed,page.page);if(!mappings.length)continue;const key=`${meta.examName}|${meta.date}`;if(!groups.has(key))groups.set(key,{key,...meta,pages:[],levels:[],mappings:[]});const g=groups.get(key);g.pages.push(page.page);g.levels.push(meta.level);g.mappings.push(...mappings)}
  ...
  function render(groups,file){
    ...
    if(sum)sum.innerHTML=`<div class="exam-auto-chip"><small>Detected exams</small><strong>${groups.length}</strong></div><div class="exam-auto-chip"><small>Orientation</small><strong>C Batch</strong></div><div class="exam-auto-chip"><small>PDF pages</small><strong>${groups.reduce((n,g)=>n+g.pages.length,0)}</strong></div><div class="exam-auto-chip"><small>Mappings</small><strong>${groups.reduce((n,g)=>n+g.mappings.length,0)}</strong></div>`;
    if(host)host.innerHTML=`<div class="multi-exams">${groups.map((g,gi)=>`<div class="multi-exam-card"><div class="multi-exam-head"><div class="multi-exam-e">E</div><div><div class="multi-exam-name">${escHtml(g.examName)}</div><div class="multi-exam-meta">${escHtml(g.levels.join(", "))} · ${escHtml(g.date||"Date not found")} · ${g.mappings.length} mappings · page${g.pages.length>1?"s":""} ${g.pages.join(", ")}</div></div></div><div class="multi-edit"><label>Exam Name<input class="multi-name" data-g="${gi}" value="${escHtml(g.editName)}"></label><label>Completion Deadline<input class="multi-deadline" data-g="${gi}" type="date" value="${escHtml(g.editDeadline)}"></label></div><details><summary>View detected syllabus</summary>${uniq(g.mappings.map(m=>`${m.level}|${m.subject}`)).map(k=>{const[level,sub]=k.split("|");const m=g.mappings.find(x=>x.level===level&&x.subject===sub);return`<div style="font-size:11px;margin:6px 0"><strong>${escHtml(level)} · ${escHtml(sub)}</strong>: ${escHtml((m?.topics||[]).join("; "))}</div>`}).join("")}</details></div>`).join("")}</div>`;
    qa(".multi-name").forEach(x=>x.oninput=()=>{state.groups[Number(x.dataset.g)].editName=x.value.trim()||state.groups[Number(x.dataset.g)].examName});qa(".multi-deadline").forEach(x=>x.onchange=()=>{state.groups[Number(x.dataset.g)].editDeadline=x.value||state.groups[Number(x.dataset.g)].deadline});
    if(btn){btn.disabled=!groups.length;btn.textContent=groups.length>1?`Save ${groups.length} Detected Exams`:`Save Detected Exam Syllabus`;btn.onclick=groups.length?save:null}
  }
  function renderDetecting(){const host=q("#examAutoMappings"),sum=q("#examAutoSummary"),btn=q("#examAutoSave"),review=q("#examAutoUiV11 .exam-auto-fallback");if(review)review.style.display="none";if(sum)sum.innerHTML="";if(host)host.innerHTML='<div class="exam-detect-mini"><span class="exam-detect-dot"></span><span>Detecting exams and syllabus…</span></div>';if(btn){btn.disabled=true;btn.textContent="Save Detected Exam Syllabus"}}
  ...
  async function save(){
    if(!state?.file||!state.groups?.length)return;const btn=q("#examAutoSave");btn.disabled=true;btn.textContent="Saving…";
    try{const file_base64=typeof fileToBase64==="function"?await fileToBase64(state.file):null;if(!file_base64)throw new Error("Could not read PDF for upload");let total=0;for(const g of state.groups){const mappings=g.mappings.map(m=>({section_id:sectionId(m.section),subject_id:subjectId(m.subject),teacher_id:teacherId(m.section,m.subject),topics:m.topics})).filter(m=>m.section_id&&m.subject_id&&m.topics.length);if(!mappings.length)continue;const r=await api("document_batch_save",{exam_name:g.editName||g.examName,exam_date:g.date||null,completion_deadline:g.editDeadline||minusDays(g.date,3)||null,file_name:state.file.name,file_size:state.file.size,file_base64,mappings});total+=r.count||mappings.length}setMsg(`Saved ${state.groups.length} exam(s) with ${total} class-subject mappings.`);const input=q("#examAutoPdf");if(input)input.value="";q("#examRefreshBtn")?.click();window.dispatchEvent(new CustomEvent("exam-syllabus-updated"))}catch(e){setMsg(e.message||String(e),true)}finally{btn.disabled=false;btn.textContent=state.groups.length>1?`Save ${state.groups.length} Detected Exams`:`Save Detected Exam Syllabus`}
  }
