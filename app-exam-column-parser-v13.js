// Authoritative Exam Syllabus parser v13: hierarchical subject headers + strict column regions.
// Physics/Chemistry may sit one header row above Track A/Track B; all are combined before slicing columns.
(function(){
  ...
  function validTopic(v){const s=clean(v);if(s.length<2||s.length>350||/^NCERT\s*:?$/i.test(s)||isMeta(s)||isPattern(s)||isHeader(s)||exactSubject(s))return false;return /[A-Za-z]/.test(s)}
  ...
  function findHeaderCluster(page,anchors){
    if(!anchors.length)return null;
    const firstRi=Math.min(...anchors.map(a=>a.ri));
    const near=anchors.filter(a=>a.ri<=firstRi+3);
    const byName=new Map();
    for(const a of near){const prev=byName.get(a.name);if(!prev||a.ri<prev.ri||(a.ri===prev.ri&&(a.j-a.i)<(prev.j-prev.i)))byName.set(a.name,a)}
    const heads=[...byName.values()].sort((a,b)=>a.cx-b.cx);
    if(heads.length<2)return null;
    return{heads,startRi:firstRi,endRi:Math.max(...heads.map(h=>h.ri))}
  }
  ...
  function isGlobalPatternStart(v){return /OBJECTIVE\s+TEST\s*[-:]?\s*\d*\s+PAPER\s+PATTERN|TOTAL\s+MARKS\s*[-:]?.*TIME/i.test(words(v))}
  function parseColumns(page,header){
    const heads=header.heads,bounds=columnBounds(heads),subjects=uniq(heads.map(h=>h.name)),topics=new Map(),lines=new Map(subjects.map(s=>[s,[]]));
    for(let ri=header.endRi+1;ri<page.rows.length;ri++){
      const row=page.rows[ri],whole=rowText(row.items);
      if(isGlobalPatternStart(whole))break;
      const laterHeads=subjectWindows(row);if(laterHeads.length>=2&&ri>header.endRi+1)break;
      for(const b of bounds){
        ...
        if(exactSubject(t)||isHeader(t)||isMeta(t)||/^NCERT\s*:?$/i.test(t))continue;
        if(isPattern(t)){const before=cutPattern(t);if(validTopic(before))lines.get(b.subject).push(before);continue}
    ...
    return{layout:"hierarchical subject columns",subjects,topics}
  }
  ...
  function parsePage(page){const anchors=allAnchors(page);if(!anchors.length)return null;const header=findHeaderCluster(page,anchors);if(header)return parseColumns(page,header);return parseRows(page,anchors)}
  ...
  function renderScanning(sig){const sum=q("#examAutoSummary"),host=q("#examAutoMappings"),btn=q("#examAutoSave");if(sum)sum.innerHTML=`<div class="exam-auto-chip"><small>Exam</small><strong>${escHtml(sig.examName)}</strong></div><div class="exam-auto-chip"><small>Orientation</small><strong>C Batch</strong></div><div class="exam-auto-chip"><small>Exam date</small><strong>Detecting…</strong></div><div class="exam-auto-chip"><small>Mapped class-subjects</small><strong>—</strong></div>`;if(host)host.innerHTML='<div class="exam-auto-empty"><strong>Reading the two-level subject header and each subject column separately…</strong><br>Physics/Chemistry and Track A/Track B are first combined into four exact column boundaries; text cannot cross columns.</div>';if(btn){btn.disabled=true;btn.onclick=null}}
  ...
  async function save(){
    if(!state?.file)return;const chosen=qa(".exam-v13-use:checked").map(x=>state.mappings[Number(x.dataset.i)]).filter(Boolean);if(!chosen.length){setMsg("Keep at least one detected mapping selected.",true);return}
